...
import { useId, useZIndex } from "@ee-ui/hooks";
import { typeIconMap } from "@ee-ui/utils";
        ...
        <div ref="rootRef" :class="[
          ...
          <div v-if="!isNil(state.title)" ref="headerRef" class="er-message-box__header"
            ...
            <div class="er-message-box__title">
              <er-icon v-if="iconComponent && state.center" :class="{
          ...
          <div class="er-message-box__content">
            <er-icon v-if="iconComponent && !state.center && hasMessage" :class="{
              [`er-icon-${state.type}`]: state.type,
            }" :icon="iconComponent" />
          ...
          <div v-show="state.showInput" class="er-message-box__input">
            <er-input v-model="state.inputValue" ref="inputRef" :placeholder="state.inputPlaceholder"
              :type="state.inputType" @keyup.enter="handleInputEnter" />
          </div>
          <div class="er-message-box__footer">
            <er-button v-if="state.showCancelButton" class="er-message-box__footer-btn er-message-box__cancel-btn"
            ...
            <er-button v-show="state.showConfirmButton" class="er-message-box__footer-btn er-message-box__confirm-btn"
              ...
              @keydown.prevent.enter="handleAction('confirm')">{{ state.confirmButtonText }}</er-button>
          </div>
        </div>
      </div>
    </er-overlay>
